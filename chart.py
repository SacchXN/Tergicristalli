...
def partial_time(fights, pull_time):
  ...
  for day in pull_time.keys():
    pt[day] = {phase: round((sum(pull[1] for pull in pull_time[day] if pull[0]==phase)/60)) for phase in fights.keys()}
...
def pie_single(fights, day, pull_time):
  pt_filtered = {k: v for k, v in pull_time.items() if v}
  ...
  plot.pie(pt_filtered.values(), [0.03]*(len(pt_filtered.keys())),  autopct = lambda p: '{:.0f}%\n{:.0f}m'.format(p,round(sum(list(pt_filtered.values()))*p/100)), 
           labels = [v[0] for v in fights_filtered.values()], startangle=90, colors = [v[1] for v in fights_filtered.values()], textprops = {'fontsize': 15})
...
async def plot_s(ctx, arg):
    ...
    try:
      for item in arg.split(','):
          urls.append(item.split('/reports/')[1].split('/')[0])
